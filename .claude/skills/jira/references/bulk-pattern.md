...
Flow for "create N issues under a parent Epic" via `jira_batch_create_issues`. Invoked when the primary skill sees subcommand `bulk` or detects bulk intent in natural language ("create 5 tasks...", "create 3 bugs...").
...
- `$2..` — topic / theme, e.g. `"refactor auth module"`. Optional but strongly recommended — without it, summaries will be generic.
...
- If `issuetype != "Epic"` → stop. Ask: `"$0 is <type>, not an Epic. Create under this parent anyway? [y/n]"`. If user declines, abort.
...
```markdown
Creating N tasks under $0 (Epic: "<summary>"):
...
Full descriptions: expand? [r]   Create? [y/n/edit]
```
...
- On partial success: show created + failed, with raw error per failed issue:
  ...
  Do you want to fix and finish the remaining? [y/n]
  ```
...
- **Duplicate summary vs siblings**: before step 4, run `jira_search(jql='parent = $0 AND summary ~ "..."')` for each planned summary. In the confirm table, add a `⚠ duplicate?` column if any match is found. Don't block — let user decide.
- **Missing env**: if `$JIRA_DEFAULT_COMPONENTS` and `$JIRA_DEFAULT_LABELS` are both empty and the user didn't provide them in the prompt, the skill must not silently omit — it asks once: `"Use empty labels and components? [y / provide values]"`. This satisfies hard invariant #2 (no default creation without explicit signal).
